...
namespace SimpleCollision {
	...
	SimpleCollision::Ball::Ball(Library::Game & game, PlayScreen* screen) :
		DrawableGameComponent(game), mBounds(Rectangle::Empty), mScreen(screen)
	{
	}

	void SimpleCollision::Ball::Initialize()
	{
		// Load a texture
		...
		mSoundEffectSimpleCollision = make_unique<SoundEffect>(mAudioEngine, L"Content\\Audio\\SimpleCollision.wav");
	...
	void SimpleCollision::Ball::Update(const Library::GameTime & gameTime)
	{
		//While in play mode...
		if (mScreen->getGameState() == GameState::Play) {
			...
			{
				mSoundEffectSimpleCollision->Play();
				//Bounce off the bottom or top>
	...
	void SimpleCollision::Ball::Draw(const Library::GameTime & gameTime)
	{
	...
	void SimpleCollision::Ball::ResetBall()
	{
	...
	const Library::Point SimpleCollision::Ball::Position() const
	{
		return mBounds.Center();
	}

	const DirectX::XMFLOAT2 SimpleCollision::Ball::Velocity() const
	{
		return mVelocity;
	}
