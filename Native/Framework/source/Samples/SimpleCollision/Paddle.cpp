...
namespace SimpleCollision {
	...
	SimpleCollision::Paddle::Paddle(Library::Game& game, PlayScreen* screen, PlayerOptions playerOption)
		: DrawableGameComponent(game), mBounds(Rectangle::Empty), mPlayerOption(playerOption), mScreen(screen)
	{
	}

	void SimpleCollision::Paddle::Initialize()
	{
		// Load a texture
	...
	void SimpleCollision::Paddle::Update(const Library::GameTime & gameTime)
	{
		//If in play mode
	...
	void SimpleCollision::Paddle::Draw(const Library::GameTime & gameTime)
	{
	...
	const Library::Rectangle SimpleCollision::Paddle::Bounds() const
	{
		return mBounds;
	}
